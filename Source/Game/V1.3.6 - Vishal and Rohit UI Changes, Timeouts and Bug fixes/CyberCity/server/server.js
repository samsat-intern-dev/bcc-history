...
io.on('connection', (socket) => {
    ...
    socket.on('restart_game', ({ clientType }) => {
        ...
        // Redirect both clients to the appropriate URL
        const redirectURL = `http://192.168.1.223:3000/`;
        console.log('***** CZ Hardcode URL ^^^^^ vvvvv');
    ...
    function handleTurnCompletion() {
        ...
        if (turnCounter % 2 === 0) {
            ...
            if (roundCounter > 1) {
                gameOver = true;
                io.emit('game_message', 'Game over after 1 rounds.');
