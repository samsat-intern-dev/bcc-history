...
const INACTIVITY_TIMEOUT = 600000; //amount of milliseconds you want prior to timeout
...
io.on('connection', (socket) => {
    ...
    socket.on('restart_game', ({ clientType }) => {
        ...
        if (restartRequests.hacker && restartRequests.defender) {
            ...
            io.emit('redirect', { url: 'http://192.168.___.___:3000' });
...
function resetInactivityTimer() {
    ...
    inactivityTimeout = setTimeout(() => {
        ...
        resetGameState();
        io.emit('game_ended', 'The game has been reset due to inactivity.');
        io.emit('redirect_to_intro');
    }, INACTIVITY_TIMEOUT);
}
